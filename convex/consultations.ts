...
export const getAvailableSlots = query({
  ...
  handler: async (ctx, args) => {
    console.log('getAvailableSlots called with:', { attorneyId: args.attorneyId, date: args.date })
    ...
    console.log('Consultation details:', existingConsultations.map(c => ({
      scheduledAt: new Date(c.scheduledAt),
      hour: new Date(c.scheduledAt).getHours(),
      status: c.status
    })))

    const bookedHours = new Set(existingConsultations.map(c => new Date(c.scheduledAt).getHours()))
    console.log('Booked hours:', Array.from(bookedHours))
    
    const availableSlots = []
    for (let hour = 9; hour <= 16; hour++) { // End at 16 (4 PM) so last slot is 4-5 PM
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.consultationType === 'hourly') {
      ...
      const pkg = attorney.fixedFeePackages.find(p => p.name === args.packageId)
      if (pkg) {
        price = pkg.price
      }
    }
    ...
    return {
      success: true,
      id: consultationId,
      message: 'Consultation booked successfully. The attorney will confirm your appointment.',
    }
  },
})
