...
export function useConsultationBooking(attorneyId: string) {
  ...
  const [consultationType, setConsultationType] = useState<'hourly' | 'fixed'>('hourly')
  ...
  useEffect(() => {
    console.log('Slots data updated:', { slotsData, isFetchingSlots, selectedDate, slotsError })
    if (slotsError) {
  ...
  const loadAvailableSlots = async (date: string) => {
    ...
    try {
      console.log('Triggering refetch for date:', date, 'attorneyId:', attorneyId)
      await refetchSlots()
