...
import { REGISTRATION_CONSTANTS, RegistrationStep } from './constants'
...
export function useAttorneyRegistration({ 
  ...
  const [formData, setFormData] = useState<Partial<AttorneyRegistrationFormData>>({
  ...
  const steps: RegistrationStep[] = ['personal', 'professional', 'practiceAndLocation', 'pricing']
  ...
  const validateCurrentStep = () => {
    try {
      ...
      setErrors({ _general: ['An unexpected error occurred during validation.'] })
      return false
    }
  }
  ...
  const updateFixedFeePackage = (index: number, updates: Partial<AttorneyRegistrationFormData['fixedFeePackages'][0]>) => {
    const currentPackages = formData.fixedFeePackages || []
    ...
    try {
      ...
      if (!result.success) {
        ...
        if (result.error && result.error.errors && Array.isArray(result.error.errors)) {
          ...
          console.warn('Unexpected final validation error structure:', result.error)
          // Fallback error handling
