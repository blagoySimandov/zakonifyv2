...
export function useReviewForm(attorneyId: string) {
  ...
  const validateForm = (): boolean => {
    ...
    } else if (formData.comment.trim().length < REVIEW_FORM_CONSTANTS.LIMITS.COMMENT_MIN) {
      newErrors.comment = [`Comment must be at least ${REVIEW_FORM_CONSTANTS.LIMITS.COMMENT_MIN} characters`]
    } else if (formData.comment.trim().length > REVIEW_FORM_CONSTANTS.LIMITS.COMMENT_MAX) {
      newErrors.comment = [`Comment must be less than ${REVIEW_FORM_CONSTANTS.LIMITS.COMMENT_MAX} characters`]
    }
  ...
  const updateFormData = (field: keyof ReviewFormData, value: string | number) => {
