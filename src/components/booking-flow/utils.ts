...
export function validatePaymentInfo(paymentInfo: PaymentInfo): ValidationErrors {
  const errors: ValidationErrors = {};
  
  if (paymentInfo.paymentMethod === 'credit-card') {
    if (!paymentInfo.cardNumber?.trim()) {
      errors.cardNumber = [BOOKING_MESSAGES.PAYMENT_STEP.VALIDATION.CARD_NUMBER_REQUIRED];
    } else if (!/^[\d\s]{13,19}$/.test(paymentInfo.cardNumber.replace(/\s/g, ''))) {
      errors.cardNumber = [BOOKING_MESSAGES.PAYMENT_STEP.VALIDATION.CARD_NUMBER_INVALID];
    }
    
    if (!paymentInfo.expiryDate?.trim()) {
      errors.expiryDate = [BOOKING_MESSAGES.PAYMENT_STEP.VALIDATION.EXPIRY_REQUIRED];
    } else if (!/^(0[1-9]|1[0-2])\/\d{2}$/.test(paymentInfo.expiryDate)) {
      errors.expiryDate = [BOOKING_MESSAGES.PAYMENT_STEP.VALIDATION.EXPIRY_INVALID];
    }
    ...
    if (!paymentInfo.cardholderName?.trim()) {
      errors.cardholderName = [BOOKING_MESSAGES.PAYMENT_STEP.VALIDATION.CARDHOLDER_NAME_REQUIRED];
    }
    
    if (paymentInfo.billingAddress) {
      if (!paymentInfo.billingAddress.street?.trim()) {
        errors.billingStreet = [BOOKING_MESSAGES.PAYMENT_STEP.VALIDATION.BILLING_ADDRESS_REQUIRED];
      }
    }
  }
