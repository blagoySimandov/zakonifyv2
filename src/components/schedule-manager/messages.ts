...
export const SCHEDULE_MESSAGES = {
  ...
  TEMPLATES: {
    ...
    TEMPLATE_DESCRIPTIONS: {
      STANDARD_BUSINESS:
        "9 AM - 5 PM, Monday to Friday with 1-hour lunch break",
      EXTENDED_HOURS:
        "8 AM - 8 PM, Monday to Friday with extended availability",
  ...
  TIME_ZONE: {
    ...
    CHANGE_WARNING:
      "Changing time zone will affect all your scheduled appointments",
  },
  ...
  VALIDATION: {
    ...
    MAX_CONSULTATIONS_RANGE:
      "Maximum consultations per day must be between 1 and 20",
  },
  ...
  TOOLTIPS: {
    BUFFER_TIME:
      "Time automatically added between consultations for preparation and transition",
    ...
    MAX_CONSULTATIONS:
      "Maximum number of consultations you want to handle per day",
    BACK_TO_BACK: "Allow consultations with no buffer time between them",
    TIME_ZONE:
      "Your local time zone - affects how clients see your availability",
  },
  ...
  UTILS: {
    TIME_STRING_TO_MINUTES: (time: string): number => {
      const [hours, minutes] = time.split(":").map(Number);
      return hours * 60 + minutes;
    },
  },

  VALIDATION_RESOLVERS: {
    WORKING_HOURS: (schedule: WeeklySchedule): string | null => {
      for (const [day, daySchedule] of Object.entries(schedule)) {
        ...
        if (endTime <= startTime) {
          return `${day}: ${SCHEDULE_MESSAGES.VALIDATION.INVALID_TIME_RANGE}`;
        }
        ...
        if (daySchedule.breaks) {
          for (const breakPeriod of daySchedule.breaks) {
            ...
            if (breakStart < startTime || breakEnd > endTime) {
              return `${day}: ${SCHEDULE_MESSAGES.BREAK_MANAGER.BREAK_VALIDATION_ERROR}`;
            }
    ...
    CONSULTATION_SETTINGS: (settings: ConsultationSettings): string | null => {
      if (settings.defaultDuration < 15 || settings.defaultDuration > 240) {
        return SCHEDULE_MESSAGES.VALIDATION.MAXIMUM_DURATION;
      }

      if (settings.bufferTime < 0 || settings.bufferTime > 60) {
        return SCHEDULE_MESSAGES.VALIDATION.INVALID_BUFFER_TIME;
      }

      if (
        settings.maxConsultationsPerDay < 1 ||
        settings.maxConsultationsPerDay > 20
      ) {
        return SCHEDULE_MESSAGES.VALIDATION.MAX_CONSULTATIONS_RANGE;
      }

      if (settings.minAdvanceBooking < 1) {
        return SCHEDULE_MESSAGES.VALIDATION.MINIMUM_ADVANCE;
      }

      if (settings.maxAdvanceBooking > 365) {
        return SCHEDULE_MESSAGES.VALIDATION.MAXIMUM_ADVANCE;
      }

      for (const consultationType of settings.consultationTypes) {
        if (consultationType.duration < 15 || consultationType.duration > 240) {
          return `${consultationType.type}: ${SCHEDULE_MESSAGES.VALIDATION.MAXIMUM_DURATION}`;
        }
