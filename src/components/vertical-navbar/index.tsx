...
export function VerticalNavbar({ attorney, isLoading, onCollapseChange, currentPage = "dashboard" }: VerticalNavbarProps) {
  ...
  if (isLoading) {
    return (
      <div className={`${isCollapsed ? 'w-16' : 'w-64'} h-screen bg-white border-r border-gray-200 flex flex-col shadow-sm fixed left-0 top-0 transition-all duration-300`}>
        <div className="p-6">
      ...
      <div className={`${isCollapsed ? 'p-3' : 'p-6'} border-b border-gray-100`}>
        <div className="text-center">
          {profileImageUrl.data ? (
            ...
            <div className={`${isCollapsed ? 'w-10 h-10' : 'w-16 h-16'} bg-blue-100 rounded-full mx-auto mb-4 flex items-center justify-center transition-all duration-300`}>
              <User className={`${isCollapsed ? 'w-5 h-5' : 'w-8 h-8'} text-blue-600`} />
            </div>
          )}
            ...
            {navigationItems.map((item) => {
              ...
              return (
                <li key={item.id}>
                  ...
                  >
                    <Icon className={`w-5 h-5 ${isActive ? "text-blue-600" : "text-gray-400"} ${isCollapsed ? '' : 'flex-shrink-0'}`} />
