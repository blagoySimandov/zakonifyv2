...
export function WorkingHoursOverview({ 
  ...
  const renderDaySchedule = (dayName: keyof WeeklySchedule, schedule: { start: string; end: string; breaks?: Array<{ start: string; end: string }> }) => {
    const shortDay = DASHBOARD_CONSTANTS.DAY_SHORT_NAMES[dayName];

    return (
      <div key={dayName} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
        ...
        <div className="text-right">
          {schedule ? (
            <div>
              ...
              {schedule.breaks && schedule.breaks.length > 0 && (
                <div className="text-xs text-gray-500 mt-1">
                  {DASHBOARD_CONSTANTS.WORKING_HOURS.BREAK_LABEL} {
            ...
            <span className="text-sm text-gray-500">
              {DASHBOARD_CONSTANTS.WORKING_HOURS.NOT_WORKING}
            </span>
          )}
        </div>
      </div>
  ...
  const hasAnySchedule = workingHours && Object.values(workingHours).some(schedule => schedule !== undefined);
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-semibold text-gray-900">
            {DASHBOARD_CONSTANTS.WORKING_HOURS.TITLE}
          </h3>
        </div>
        <button
          onClick={onEditClick}
        ...
        >
          ...
          {DASHBOARD_CONSTANTS.WORKING_HOURS.EDIT_BUTTON}
        </button>
      </div>
      ...
      {timeZone && (
        <div className="flex items-center gap-2 text-sm text-gray-600 mb-4 pb-4 border-b border-gray-100">
          ...
          <span>
            {DASHBOARD_CONSTANTS.WORKING_HOURS.TIME_ZONE_LABEL} {timeZone.replace('_', ' ')}
          </span>
        </div>
      )}
      ...
      {hasAnySchedule ? (
        ...
        <div className="text-center py-8">
          ...
          <p className="text-gray-500 text-sm">
            {DASHBOARD_CONSTANTS.WORKING_HOURS.NO_SCHEDULE}
          </p>
          <button
            onClick={onEditClick}
